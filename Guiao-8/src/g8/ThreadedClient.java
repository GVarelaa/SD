        ...
        Thread[] threads = {
            ...
            new Thread(() -> {
                try  {
                    ...
                    Thread.sleep(100);
                    // get reply
                    byte[] data = m.receive(1); // bloqueia enquanto nao existirem mensagens / erro
            ...
            new Thread(() -> {
                try  {
                    ...
                    Thread.sleep(100);
                    // get reply
                    byte[] data = m.receive(3); // bloqueia enquanto nao existirem mensagens / erro
            ...
            new Thread(() -> {
                try  {
                    ...
                    for (;;) {
                        byte[] data = m.receive(2); // bloqueia enquanto nao existirem mensagens / erro
                        if (data.length == 0)
                            break;
            ...
            new Thread(() -> {
                try  {
                    ...
                    for (;;) {
                        byte[] data = m.receive(4); // bloqueia enquanto nao existirem mensagens / erro
                        if (data.length == 0)
                            break;
